...
const ProductDetail = () => {
    ...
    const handleAddToCartClick = async () => {
        ...
        if (user.role !== 'buyer') return alert('Only buyers can add to cart'); // Simple for now
        if (!activeVariant) return alert('Please select all options');
        if (activeVariant.stock <= 0) return alert('Out of stock');

        setIsAdding(true);
        try {
            const variantIndex = product.variants.indexOf(activeVariant);
            await handleAddToCart(product._id, variantIndex, 1);
            alert('Added to your archive');
        } catch (error) {
            alert(error.message || 'Failed to add to archive');
        } finally {
            setIsAdding(false);
        }
    };
    ...
    const handleBuyNow = async () => {
        if (!user) return navigate('/login');
        if (user.role !== 'buyer') return alert('Only buyers can purchase');
        if (!activeVariant) return alert('Please select all options');
                    ...
                    <div className="flex gap-6 items-center">
                        {user ? (
                            <>
                                <UserMenu user={user} />
                                {user.role === 'buyer' && (
                                    <Link to="/cart" className="text-[10px] uppercase tracking-[0.2em] font-medium transition-colors hover:text-[#C9A96E]" style={{ color: '#7A6E63' }}>
                                        Bag
                                    </Link>
                                )}
                            </>
                        ...
                        <div className="w-full lg:w-[30%] lg:sticky lg:top-24 flex flex-col pt-4">
                            ...
                            <div className="flex flex-col gap-4 mt-auto">
                                <button
                                    ...
                                    onMouseEnter={e => {
                                        if (!isAdding && activeVariant?.stock > 0) {
                                ...
                                >
                                    {isAdding ? 'Adding...' : activeVariant?.stock > 0 ? 'Add to Cart' : 'Out of Stock'}
                                </button>

                                <button
                                    onClick={handleBuyNow}
                                    disabled={activeVariant?.stock <= 0}
                                    className="w-full py-4 text-[11px] uppercase tracking-[0.25em] font-medium transition-all duration-300 border disabled:opacity-50"
                                    style={{
